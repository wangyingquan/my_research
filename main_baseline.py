...
def train(model, trainloader, xent, tent, optimizer, use_gpu):
    ...
    for batch_idx, (imgs, pids, _) in enumerate(trainloader):
        if use_gpu:
            imgs = imgs.cuda()
        apprance_loss, outputs, features = model(imgs)
        ...
        apprance_losses.update(apprance_loss.item(),1)
    ...
    # print("Batch {}/{}\t Loss {:.6f} ({:.6f}) xent Loss {:.6f} ({:.6f}), appearance Loss {:.6f} ({:.6f})".format(
    #     batch_idx + 1, len(trainloader), losses.val, losses.avg, xent_losses.val, xent_losses.avg, apprance_losses.val,
    #     apprance_losses.avg))
    # print("Batch {}/{}\t Loss {:.6f} ({:.6f}) xent Loss {:.6f} ({:.6f})".format(batch_idx + 1, len(trainloader), losses.val, losses.avg, xent_losses.val, xent_losses.avg))
